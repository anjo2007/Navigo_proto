...
import { streamRoutes, searchNearbyPlaces, hasGeminiKey } from '../services/geminiService';
...
const RoutePlanner: React.FC<RoutePlannerProps> = ({ user, onLoginClick, onLogoutClick, onAdminClick, onProfileClick, onAddBusClick }) => {
  ...
  // Check API Key on first load if user hasn't configured it
  useEffect(() => {
    if (!hasGeminiKey()) {
      // Delay slightly for smooth initial rendering
      const timer = setTimeout(() => setShowKeyModal(true), 1200);
  ...
  const handleSearch = useCallback(async () => {
    ...
    setLoadingMessage("NaviGo AI optimizing multi-modal routes...");
    ...
    const onError = (err: Error) => {
        ...
        if (err?.message === "GEMINI_KEY_MISSING") {
          ...
          setError("Routing failed. Please verify API Key & connection.");
        }
    };
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-void">
      ...
      <Header 
        ...
        onGeminiKeyClick={() => setShowKeyModal(true)}
      />
