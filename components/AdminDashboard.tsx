...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        // Real-time subscription via Firebase Firestore or fallback
        const unsubscribe = databaseService.subscribeToReports((updatedReports) => {
    ...
    const handleValidation = async (id: string, isValid: boolean) => {
        ...
        setReports(prev => prev.map(r => r.id === id ? { ...r, status: isValid ? 'APPROVED' : 'REJECTED' } : r));
                ...
                <div className="bg-[#1E1E1E] p-6 flex justify-between items-center border-b border-white/10">
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-ash">System Integrity & Data Verification • <span className="text-neon animate-pulse">● Live Firebase Ingest</span></p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-[#121212]">
                    {activeTab === 'reports' && (
                        <div className="space-y-4">
                            {reports.length === 0 && (
                                <div className="text-center py-20 text-ash">No pending reports to validate.</div>
                            )}
                            {reports.map((report) => (
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-ash mt-1">
                                                User: <span className="text-azure">{report.userId.substring(0,12)}...</span> • {new Date(report.timestamp).toLocaleTimeString()}
                                            </p>
                    ...
                    {activeTab === 'bulk' && (
                        <div className="flex flex-col items-center justify-center h-full text-ash border-2 border-dashed border-white/10 rounded-xl p-10">
                           <p className="text-white font-bold mb-2">Bulk CSV Route Ingestion Module</p>
                           <p className="text-xs text-ash max-w-sm text-center">Upload GTFS or CSV bus schedules directly into the Firebase database.</p>
                        </div>
                    )}
                </div>
            </div>
